...
sleep 10
# mysql -h mariadb_michelle -u root -pmichelle123

if ! mysql -h mariadb_michelle -u root -pmichelle123 -e 'USE baseballdb'; then
    echo "    Could not find baseballdb. Creating data base"
    mysqladmin -u root -h mariadb_michelle -pmichelle123 create baseballdb
    echo "    Importing database from ubuntu container"
    mysql -u root -pmichelle123 -h mariadb_michelle --database=baseballdb < baseball.sql
fi
...
mysql -h mariadb_michelle -u root -pmichelle123 -e '
 USE baseballdb;
...
mysql -h mariadb_michelle -u root -pmichelle123 -e '
  USE baseballdb;
  SELECT * FROM rolling_avg;' > /app/output/results.txt
